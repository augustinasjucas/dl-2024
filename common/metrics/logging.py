...
class BeautifulLogging(LossAccuracyCalculator):
    def __init__(self, wandb_params):
        super().__init__(
            "BeautifulLogging",
            "Logs the training progress to wandb, with each task being a separate RUN!",
        )
        self.wandb_params = wandb_params

    def before_task(self, model, task_index, train_loader, test_loader):
        self.current_task_logger = wandb.init(
            **self.wandb_params, name=f"sequential-training-task-{task_index}"
        )
    ...
    def after_task(self, model, task_index, train_loader, test_loader):
        # Test on this task
        test_loss, test_accuracy = simple_test(
            model, test_loader, self.criterion, self.device
        )
        train_loss, train_accuracy = simple_test(
            model, train_loader, self.criterion, self.device
        )

        wandb.log(
            {
                "primary-sequential-training/test-loss": test_loss,
                "primary-sequential-training/test-accuracy": test_accuracy,
                "primary-sequential-training/train-loss": train_loss,
                "primary-sequential-training/train-accuracy": train_accuracy,
            }
        )

        # Print the results for this task
        print(
            f"    After training on task {task_index}: Test Loss: {test_loss}. Test Accuracy: {test_accuracy}, Train Loss: {train_loss}. Train Accuracy: {train_accuracy}"
        )
    ...
    def after_epoch(self, model, task_num, epoch_num):
        ...
        test_loss, test_accuracy = simple_test(
            model, self.test_loader, self.criterion, self.device
        )

        # Log the epoch results to wandb
        wandb.log(
            {
                "primary-sequential-training/running-train-loss": self.epoch_loss
                / len(self.train_loader.dataset),
                "primary-sequential-training/running-train-accuracy": self.epoch_accuracy
                / len(self.train_loader.dataset),
                "primary-sequential-training/running-test-loss": test_loss,
                "primary-sequential-training/running-test-accuracy": test_accuracy,
            }
        )
